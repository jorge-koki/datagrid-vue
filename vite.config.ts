 ...
 * `import 'vue-tablekit/style.css'` de la demo resuelve al módulo que ya estaba
...
export default defineConfig(({ mode }) => {
  ...
  return {
    ...
    resolve: {
       ...
       * con la primera que coincide: `'vue-tablekit'` puesto antes se tragaría
       * `vue-tablekit/style.css` y lo reescribiría como `<index.ts>/style.css`.
       * La entrada más específica va primero.
       */
      alias: {
        'vue-tablekit/style.css': LIB_STYLE,
        'vue-tablekit': LIB_ENTRY,
      },
    },
