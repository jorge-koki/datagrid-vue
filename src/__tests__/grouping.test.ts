...
describe('pool — a group header is a row node built once and mutated after', () => {
  ...
  it('indents by depth with a single custom property, not with nested wrappers', () => {
    ...
    // La sangría no agrega nodos: la banda tiene su caja interior, y esa caja
    // tiene chevrón, etiqueta y contador, y nada más.
    expect(headers[1]?.querySelector('.dt-group-header')?.children).toHaveLength(1)
    expect(headers[1]?.querySelector('.dt-group-header-inner')?.children).toHaveLength(3)
    fixture.destroy()
  })
  ...
  it('follows the resolved columns when the horizontal window moves', () => {
    ...
    // El tramo visible se corre: entra `progress`, que también agrega, y cada
    // agregado se reubica. Los offsets salen de las columnas ya resueltas, así
    // que resize, reorden y ocultamiento los arrastran solos.
    fixture.paint({ columns: resolved.slice(1, 4), totalWidth: 360 })
    aggregates = [
      ...(groupNodes(fixture.container)[0]?.querySelectorAll('.dt-group-aggregate') ?? []),
    ].filter((node): node is HTMLElement => node instanceof HTMLElement && !node.hidden)
    ...
    // La banda NO sigue al tramo visible: mide la fila entera y arranca en su
    // borde izquierdo. Es la diferencia entre cubrir el bloque anclado y dejarlo
    // como un hueco, porque las columnas ancladas nunca están en `columns`.
    const header = groupNodes(fixture.container)[0]?.querySelector('.dt-group-header')
    expect(header instanceof HTMLElement && header.style.transform).toBe('')
    expect(header instanceof HTMLElement && header.style.width).toBe('360px')

    fixture.destroy()
  })

  it('spans the whole row, so pinned columns are covered and not left blank', () => {
    const rows = demoRows(6)
    const columns: readonly DataTableColumn<DemoRow>[] = [
      { key: 'name', width: 100 },
      { key: 'status', width: 80 },
    ...
    // Se pinta solo el tramo que scrollea, que es lo que pasa de verdad cuando
    // hay una columna anclada: la anclada sale de la ventana virtual para no
    // pintarse dos veces, así que `columns[0]` empieza DESPUÉS de ella.
    fixture.paint({ columns: resolved.slice(1), totalWidth: 300 })

    const header = groupNodes(fixture.container)[0]?.querySelector('.dt-group-header')
    expect(header).toBeInstanceOf(HTMLElement)
    if (header instanceof HTMLElement) {
      // Cero, no 100: el ancho de la columna que quedó fuera del tramo.
...
describe('component — grouping plugged into everything that already existed', () => {
  it('becomes a treegrid while grouping is active and a grid when it is not', async () => {
    ...
    expect(grouped.grid.querySelector('.dt-header-row')?.getAttribute('role')).toBe('row')
