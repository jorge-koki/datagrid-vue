...
- [x] **Nombre** `vue-tablekit`, libre y sin lápida en el registro. Ver arriba.
...
- [x] **`files: ["dist"]`**, con el contenido exacto del tarball verificado en CI por
      `npm pack --dry-run`.
- [x] Script **`prepare`**, para que `npm install github:jorge-koki/vue-tablekit` construya `dist/` al
...
- [x] **Workflow de publicación** (`.github/workflows/release.yml`), disparado por un tag `vX.Y.Z`,
      que verifica que el tag y `package.json` digan lo mismo antes de publicar.
...
- [ ] **Saber qué cuesta `prepare`.** También corre en cada `npm install` local de este repositorio, y
      una falla del build hace fallar la instalación. Se deja igual a propósito: es la misma ruta que
      ejecuta quien instala desde GitHub, así que cada `npm ci` de CI la verifica. La alternativa es
      versionar `dist/` y sacar el script.

Publicar no necesita ningún paso extra: `npm publish` corre `prepublishOnly` —tipos y tests— y
después `prepare`, que construye `dist/`, y `files: ["dist"]` deja todo lo demás afuera del tarball.
